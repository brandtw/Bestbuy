# To do: beeper, threading, continuously click "please wait" IF it appears, address comments below
...
wait = WebDriverWait(browser,25)                        # <-- Set webpage timeout to max 25 seconds                                                                     
...
def my_function(address):
    ...
    browser.find_element_by_xpath('//*[@id="large-footer"]/div[2]/div[1]/div[1]/ul/li[3]').click            # I CANNOT FIGURE OUT HOW TO HIT THE SIGN IN BUTTON FOR THE LIFE OF ME
    ...
    while not buyButton:        
        try:
            outofstockbtn = browser.find_element_by_xpath("//button[contains(@class, 'btn-disabled')]") # Research way to keep code from waiting for website to load?
            browser.refresh()
        except:
            try:                                                        
